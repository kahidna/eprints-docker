...
# install eprints if necessary and start webserver
SRCPATH=/tmp
INSTALLPATH=/opt/eprints3

if [[ -z "$INSTALLPATH" ]]; then
	#statements
	echo "eprints is not installed on $INSTALLPATH"
	echo "start installing"
	cd $SRCPATH && wget https://files.eprints.org/2306/5/eprints-$EPRINTSV.tar.gz && \
	tar -xvf eprints-$EPRINTSV.tar.gz && cd eprints-$EPRINTSV && \
	./configure && make install && echo "installation done"
	echo
else
	echo "all is good"
